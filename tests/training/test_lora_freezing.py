...
def test_freeze_keeps_agent_embeddings_trainable(tiny_model, matrix_config):
    ...
    # Agent embeddings still trainable, as are the (always-present) inactive-cell
    # embedding and activity head -- all part of the matrix interface.
    ...
    expected = (
        model.agent_embeddings.weight.numel()
        + model.inactive_embedding.weight.numel()
        + model.activity_head.weight.numel()
        + model.activity_head.bias.numel()
    )
